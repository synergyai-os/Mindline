...
The frozen public scorecard at `.productbrain/specs/fixtures/wp53-readonly-beta-scorecard-v1.json`, SHA-256 `3694ecc6e7006a141b459ea2e76cc787b1ea6c81a0064eb95d0cda2c1475a7df`, contains eight paraphrased answerable cases, four context-bound known-absent cases, four owner-approved contexts, exact per-case supporting-evidence answer keys, exact shared-membership/order answers, a fully specified multi-resource hydration-isolation case, and one fixed measurement profile. The supporting-evidence answer keys are test-only: production ranking and authorization may never read case IDs, expected IDs, or allowed-supporting IDs. Before retrieval implementation, record the exact `origin/main` result. The exact merge candidate must prove:
...
2. All 4 absent cases abstain. For every answerable case, every returned citation independently passes query-only eligibility, belongs to that case's frozen `allowed_supporting_record_ids`, and contains no unknown or duplicate ID. This evaluation answer key never becomes runtime authorization.
3. Every returned citation hydrates through the same run and context to its exact qualifying source. Both the search packet and hydrated capture exclude sibling and historical IDs, URLs, hashes, metadata, and content. Search and get use one current-resource reachability rule, including follow-up resources.
...
- The founder's direct 2026-08-31 instruction to update the goal and finish from `main` authorizes this superseding evaluator-answer-key and exact-source-isolation correction, one accepted and verified governing decision over its exact hashes, and one unchanged-artifact full-panel rerun. No further correction is allowed: a remaining material blocker stops. The corrected implementation gets one mandatory clean rereview. A new concern outside this threat model becomes follow-up Chain work, not silent scope expansion.
