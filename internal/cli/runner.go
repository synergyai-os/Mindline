...
import (
	...
	"github.com/synergyai-os/Mindline/internal/evalloopdecision"
...
var usage = strings.NewReplacer(
	...
	"usage: mindline memory import-slack <native-batch.json|-> [--root <private-dir>]\n",
	"usage: mindline memory ingest-slack-run - [--root <private-dir>]\nusage: mindline memory ingest-slack-run-status [--root <private-dir>]\nusage: mindline memory ingest-slack-run-proof [--root <private-dir>]\nusage: mindline memory resources-run [--root <private-dir>]\nusage: mindline memory resources-continue [--root <private-dir>]\nusage: mindline memory resources-reconcile [--root <private-dir>]\nusage: mindline memory resources-retry --reason unreachable|rate_limited [--root <private-dir>]\nusage: mindline memory resources-status [--root <private-dir>]\nusage: mindline memory resources-proof [--root <private-dir>]\nusage: mindline memory resources-rebuild-proof [--root <private-dir>]\nusage: mindline memory founder-review - [--root <private-dir>]\nusage: mindline memory import-slack <native-batch.json|-> [--root <private-dir>]\n",
...
func NewRunnerWithProtectedRoots(fileSystem FileSystem, protectedRoots []string) Runner {
	...
	return Runner{fs: fileSystem, agentExecutable: executable, agentNamespace: "agent", nativeInput: os.Stdin, operatorInput: os.Stdin, protectedRoots: append([]string(nil), protectedRoots...), agentDiscoveryTimeout: 2 * time.Second, agentRegistrationTimeout: 10 * time.Second}
}
...
func (r Runner) Run(args []string, stdout, stderr io.Writer) int {
	...
	if args[0] == "agent-only" {
		return r.runAgentOnly(args[1:], stdout, stderr)
	}
	if args[0] == "slack" {
		return r.runSlack(args[1:], stdout, stderr)
	}
	if args[0] == "gmail" {
		return r.runGmail(args[1:], stdout, stderr)
	}
...
func (r Runner) runDocuments(args []string, stdout, stderr io.Writer) int {
	...
	if len(args) > 0 && args[0] == "meaning-packet" {
		return r.runDocumentsMeaningPacket(args, stdout, stderr)
	}
	if len(args) > 0 && args[0] == "concept-index" {
		return r.runDocumentsConceptIndex(args, stdout, stderr)
	}
	if len(args) > 0 && args[0] == "concept-serve" {
		return r.runDocumentsConceptServe(args, stdout, stderr)
	}
	if len(args) > 0 && args[0] == "value-proof" {
		return r.runDocumentsValueProof(args, stdout, stderr)
	}
...
func (r Runner) runEval(args []string, stdout, stderr io.Writer) int {
	...
	if len(args) > 0 && args[0] == "proof-gate" {
		return r.runEvalProofGate(args, stdout, stderr)
	}
	if len(args) > 0 && args[0] == "loop-decision" {
		return r.runEvalLoopDecision(args, stdout, stderr)
	}
	fmt.Fprint(stderr, usage)
	return ExitUsage
}
...
func (r Runner) runEvalProofGate(args []string, stdout, stderr io.Writer) int {
	...
	packet, err := evalproof.Build(inputRoot, outDir, evalproof.Options{BaselineRoot: baselineRoot, Claim: claim, ProtectedRoots: r.protectedRoots})
	if err != nil {
		fmt.Fprintf(stderr, "eval proof-gate: %v\n", err)
		return ExitProcess
	}
	encoder := json.NewEncoder(stdout)
	encoder.SetIndent("", "  ")
	...
	if packet.ExitCode != ExitOK {
		return ExitProcess
	}
	return ExitOK
}

func (r Runner) runEvalLoopDecision(args []string, stdout, stderr io.Writer) int {
	...
	packet, err := evalloopdecision.Build(inputRoot, outDir, evalloopdecision.Options{BaselineRoot: baselineRoot, ProtectedRoots: r.protectedRoots})
	if err != nil {
		fmt.Fprintf(stderr, "eval loop-decision: %v\n", err)
		return ExitProcess
	}
	encoder := json.NewEncoder(stdout)
	encoder.SetIndent("", "  ")
...
func (r Runner) runDocumentsCorpusAcceptanceLabeling(args []string, stdout, stderr io.Writer) int {
	pressurePath, outDir, options, parseError := parseDocumentsCorpusAcceptanceLabelingArgs(args)
	if parseError != parseErrorNone {
		fmt.Fprint(stderr, usage)
		return ExitUsage
	}
	...
	packet, _, seed, err := documents.BuildCorpusAcceptanceLabelingPacketWithOptions(pressurePath, outDir, options)
	if err != nil {
		if documents.IsArtifactWriteError(err) {
	...
	encoder := json.NewEncoder(stdout)
	encoder.SetIndent("", "  ")
	if err := encoder.Encode(documents.CorpusAcceptanceLabelingOutputForSeed(packet, seed)); err != nil {
		fmt.Fprintf(stderr, "write stdout: %v\n", err)
		return ExitUsage
	}
	return ExitOK
...
func (r Runner) runDocumentsMeaningPacket(args []string, stdout, stderr io.Writer) int {
	inputPath, outDir, parseError := parseDocumentsArgs(args, "meaning-packet")
	if parseError != parseErrorNone {
		fmt.Fprint(stderr, usage)
		return ExitUsage
	}
	if err := r.validateDestinationOutDir(outDir); err != nil {
		fmt.Fprintf(stderr, "validate meaning packet output: %v\n", err)
		return ExitArtifactWrite
	}
	summary, _, err := documents.BuildSourceMeaningPacket(inputPath, outDir)
	if err != nil {
		if documents.IsArtifactWriteError(err) {
			fmt.Fprintf(stderr, "write meaning packet: %v\n", err)
			return ExitArtifactWrite
		}
		fmt.Fprintf(stderr, "build meaning packet: %v\n", err)
		return ExitProcess
	}
	encoder := json.NewEncoder(stdout)
	encoder.SetIndent("", "  ")
...
func (r Runner) runDocumentsConceptIndex(args []string, stdout, stderr io.Writer) int {
	inputPath, outDir, parseError := parseDocumentsArgs(args, "concept-index")
	if parseError != parseErrorNone {
		fmt.Fprint(stderr, usage)
		return ExitUsage
	}
	if err := r.validateDestinationOutDir(outDir); err != nil {
		fmt.Fprintf(stderr, "validate concept index output: %v\n", err)
		return ExitArtifactWrite
	}
	summary, _, err := documents.BuildCorpusConceptIndex(inputPath, outDir)
	if err != nil {
		if documents.IsArtifactWriteError(err) {
			fmt.Fprintf(stderr, "write concept index: %v\n", err)
			return ExitArtifactWrite
		}
		fmt.Fprintf(stderr, "build concept index: %v\n", err)
		return ExitProcess
	}
	encoder := json.NewEncoder(stdout)
	encoder.SetIndent("", "  ")
...
func (r Runner) runDocumentsConceptServe(args []string, stdout, stderr io.Writer) int {
	inputPath, addr, parseError := parseDocumentsConceptServeArgs(args)
	if parseError != parseErrorNone {
		fmt.Fprint(stderr, usage)
		return ExitUsage
	}
	...
	if err != nil {
		fmt.Fprintf(stderr, "serve corpus concept UI: %v\n", err)
		return ExitProcess
	}
	defer listener.Close()
	fmt.Fprintf(stdout, "Mindline concept review UI: http://%s\n", listener.Addr().String())
	if err := http.Serve(listener, newCorpusConceptUIHandlerWithAllowedHosts(inputPath, []string{addr, listener.Addr().String()})); err != nil {
		fmt.Fprintf(stderr, "serve corpus concept UI: %v\n", err)
		return ExitProcess
	}
	return ExitOK
...
func (r Runner) runDocumentsValueProof(args []string, stdout, stderr io.Writer) int {
	inputPath, outDir, options, parseError, configError := r.parseDocumentsValueProofArgs(args)
	if configError != "" {
		fmt.Fprintln(stderr, configError)
		return ExitUsage
	}
	...
	if err := r.validateDestinationOutDir(outDir); err != nil {
		fmt.Fprintf(stderr, "validate value proof output: %v\n", err)
		return ExitArtifactWrite
	}
	summary, err := documents.BuildValueProof(inputPath, outDir, documents.ValueProofOptions{
		PressureOptions: documents.CorpusPressureOptions{SemanticOptions: options},
	})
	if err != nil {
		if documents.IsArtifactWriteError(err) {
			fmt.Fprintf(stderr, "write value proof: %v\n", err)
			return ExitArtifactWrite
		}
		fmt.Fprintf(stderr, "build value proof: %v\n", err)
		return ExitProcess
	}
	encoder := json.NewEncoder(stdout)
	encoder.SetIndent("", "  ")
...
func (r Runner) runGmail(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 || args[0] != "corpus-intake" {
		fmt.Fprint(stderr, usage)
		return ExitUsage
	}
	inputPath, outDir, parseError := parseDocumentsArgs(args, "corpus-intake")
	...
	input, err := r.fs.ReadFile(inputPath)
	if err != nil {
		fmt.Fprintf(stderr, "read Gmail export: %v\n", err)
		return ExitUsage
	}
	var payload gmailadapter.Payload
	if err := json.Unmarshal(input, &payload); err != nil {
		fmt.Fprintf(stderr, "build Gmail corpus intake: %v\n", err)
		return ExitProcess
	}
	summary, err := gmailadapter.BuildCorpusIntakeWithFileSystem(payload, outDir, r.fs)
	if err != nil {
		fmt.Fprintf(stderr, "build Gmail corpus intake: %v\n", err)
		return ExitArtifactWrite
	}
...
func parseEvalProofGateArgs(args []string) (inputRoot string, baselineRoot string, outDir string, claim string, err parseError) {
	if len(args) < 6 || args[0] != "proof-gate" || strings.TrimSpace(args[1]) == "" {
		return "", "", "", "", parseErrorUsage
	}
	inputRoot = args[1]
	for i := 2; i < len(args); {
	...
	return inputRoot, baselineRoot, outDir, claim, parseErrorNone
}

func parseEvalLoopDecisionArgs(args []string) (inputRoot string, baselineRoot string, outDir string, err parseError) {
	if len(args) < 4 || args[0] != "loop-decision" || strings.TrimSpace(args[1]) == "" {
		return "", "", "", parseErrorUsage
	}
	inputRoot = args[1]
	for i := 2; i < len(args); {
...
func (r Runner) parseDocumentsValueProofArgs(args []string) (inputPath string, outDir string, options documents.SemanticOptions, err parseError, configError string) {
	options.Classifier = documents.SemanticClassifierDeterministic
	if len(args) < 4 || args[0] != "value-proof" || strings.TrimSpace(args[1]) == "" {
		return "", "", options, parseErrorUsage, ""
	}
	inputPath = args[1]
	for i := 2; i < len(args); {
...
func parseDocumentsCorpusAcceptanceLabelingArgs(args []string) (pressurePath string, outDir string, options documents.CorpusAcceptanceLabelingOptions, err parseError) {
	...
	for i := 2; i < len(args); {
		...
		case "--max-cases":
			...
			maxCases, convErr := strconv.Atoi(args[i+1])
			if convErr != nil || maxCases <= 0 {
				return "", "", options, parseErrorUsage
			}
	...
	if options.MaxCases > 0 && !options.Seed {
		return "", "", options, parseErrorUsage
	}
	return pressurePath, outDir, options, parseErrorNone
}
...
func parseDocumentsConceptServeArgs(args []string) (inputPath string, addr string, err parseError) {
	if len(args) < 2 || args[0] != "concept-serve" || strings.TrimSpace(args[1]) == "" {
		return "", "", parseErrorUsage
	}
	inputPath = args[1]
	addr = "127.0.0.1:8788"
	...
	return inputPath, addr, parseErrorNone
}
